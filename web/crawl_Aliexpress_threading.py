...
class crawl:
	...
	def crawl_single_url(slef, threadName,id):
		productid = id
		...
		try:
			...
			row.append(wholeCategory)
			outputQ.put(row)
			# Should get into the queue here------------------>>>>>>>>>
...
class myThread (threading.Thread):   #ca threading.Thread
    ...
    def __init__(self, threadID, name, q):
        ...
        self.name = name
        self.q = q
    def run(self):                   #run will be the auto process when thread is running
        print "Starting " + self.name
        while not self.q.empty():
        	id = self.q.get()
        	crawl_single_url(self.name, id)
...
def buildHeaders(id):
	headers={
		...
		'cookie':cookie,
		'dnt': '1',
		#'referer': 'https://sale.aliexpress.com/__pc/brands_YI_2019Mid_YearSale.htm?spm=2114.best.100003.1.62665ekN5ekNIO&gps-id=5209001&scm=1007.15875.101669.0&scm_id=1007.15875.101669.0&scm-url=1007.15875.101669.0&pvid=7a97be63-c6ea-47f1-b789-966f966c5181',
		'upgrade-insecure-requests': '1',
...
def write_to_csv(queue,outputFile):
...
def crawl_single_url(threadName,id):
	if exitFlag:
		(threading.Thread).exit()
	...
	try:
		...
		res = requests.get("https://www.aliexpress.com/item/" + productid + '.html', headers = headers)
		soup = BeautifulSoup(res.text, 'html.parser')
		title = soup.findAll("meta", {"property" : "og:image"})
		part_url = title[0]['content'].split('/')[-1].split('.')[0]
		url = "https://www.aliexpress.com/item/" + part_url +'/' + productid + '.html'
		...
		imgUrl = soup.select('.ui-image-viewer-thumb-frame > img')[0].attrs['src']
		img = requests.get(imgUrl)
		fname = 'f' + str(productid)
		with open('./pics/' + str(productid) + '.jpg','wb') as fname:
			fname.write(img.content)
		wholeCategory = ''
		for c in parent_category:
			wholeCategory += c.text.replace(",","") + ' > '
		...
		print "Id: "+ productid+" not exist!"

WholeProcessStart = datetime.datetime.now()
...
exitFlag = 0
outputQ = Queue.Queue()
head = ['Product_ID', 'Url', 'Title', 'Category', 'Img']
outputQ.put(head)
threadlist = []
# create threads
for i in range(0,10):
	#thread = "thread" + str(i)
	thread = myThread(i, "Thread-" + str(i), workQ)
	threadlist.append(thread)
	
# start threads dai bi 
for i in range(0,10):
	threadlist[i].start()
...
write_to_csv(outputQ,outputFile)
WholeProcessEnd = datetime.datetime.now()
print (WholeProcessEnd - WholeProcessStart)